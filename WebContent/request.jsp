...
<title>Insert title here</title>
        ...
        <style>
            ...
            *{font-family:"barunGothic";font-size:14px;}
            ...
            .container{width: 100%;}
            .container>div{padding-top: 10px;}
            .table-wrapper .row>div{padding-left: 0px;padding-right: 0px;}
            .table-wrapper .row{padding:10px;}
            .top-btn>.row>div{padding-left: 0px;padding-right: 0px;}
            .search>div{padding-left: 0px;padding-right: 0px;}
            .text-right>div{padding-left: 0px;padding-right: 0px;}
            .contents>div{padding-top: 30px;}
            .page-link{color:#f56a6a;} /* 색상 설정 */
            .btn{background-color:#f56a6a;}
            .search .btn{width: 100%;}
            .table-wrapper .row{border-bottom: 1px solid #dddddd;}
            .table-wrapper .row>div{padding: 8px;}
            .left{border-bottom: 1px solid #dddddd;border-right: 1px solid #dddddd;border-left: 1px solid #dddddd;}
            .right{border-bottom: 1px solid #dddddd;}
            .fa-pen-nib{color:#f56a6a;}
            .top-btn>.row{padding:10px;border-bottom:1px solid #dddddd;margin:10px;}
            .toWrite{background-color:white;border:0px;}
            .toWrite h4{margin-bottom:0px;}
            .toWrite:hover{border-bottom:1px solid #f56a6a;}
            button.sortOption{
				display:inline;
				border:0px;
				background-color:white;
            ...
            .sort .sortbox{position:absolute;bottom:5px;}
            .sort button.active{border-bottom: 1px solid #333;}
            .sortContents{display:none;}
            #board_title a b{font-size:20px;text-decoration:none;color:black;}
        ...
        <script>
            $(function(){
                ...
                $("#toWrite").on("click",function(){
                	location.href = "request_write.jsp";
                	if($("#idSession").text() == ""){
	            		alert("회원가입이 필요합니다.");
	            		result = false;
	...
	<body>
		<div id="idSession" style="display:none;">${sessionScope.clientid}</div>
             <!-- header 영역 (맨위에 메뉴바) -->
                ...
                <!-- <form action="search.req" method="get" id="searchform"> -->
	                <div class="row search">
	                    ...
	                    <div class="col-7 col-sm-8 col-lg-9 search-box">
	                        <input type="text" class="form-control input" name="keyword" id="keyword" placeholder="키워드를 입력하세요." aria-label="Recipient's username" aria-describedby="button-addon2">
	                    </div>
	                ...
	                <div class="row">
								...
								<c:otherwise>
										...
										<c:choose>
			                            	<c:when test="true">
			                            			<div class="col-8 text-left sort">
			                            				<div class="sortbox">
			                            					<button type=button class="sortOption" value="newRequest">최신 질문순</button>
			                            					·
			                            					<button type=button class="sortOption" value="view">조회순</button>
			                            				</div>
			                            			</div> 
					                        		<div class="col-4 text-right"><button type="submit" class="btn btn-danger" id="delete">체크한 글 삭제</button></div>                       	
			                            	</c:when>
			                            	<c:otherwise>
		                        	...
		                        	<c:forEach var="dto" items="${list}">
			                        	<div class="row">
			                                ...
			                                <div class="col-6 text-left" id="board_title"><a href="contents.req?seq=${dto.seq}"><b>${dto.title}</b></a></div>
			                                <c:choose>
			                                	<c:when test="true">
			                                		<div class="col-6 text-right"><input type="checkbox" class="delete" name="deleteSeq" value="${dto.seq}"></div>
			                                	</c:when>
			                                	<c:otherwise>
			                                ...
			                                <c:choose>
			                                	<c:when test="${empty cmtList[dto.seq]}">
					                                ...
					                                <div class="col-12 text-left" id="board_view">조회수 ${dto.view_count}</div>
			                                	</c:when>
			                                	<c:otherwise>
			                                		<div class="col-12 text-left" id="cmt_writer">답변 ${cmtList[dto.seq][0].writer} 변호사</div>
					                                <div class="col-12 text-left" id="cmt_contents">${cmtList[dto.seq][0].contents}</div>
					                                <div class="col-12 text-left" id="cmt_count">다른 변호사 답변 ${cmtList[dto.seq].size()-1} 개</div>
					                                <div class="col-6 text-left" id="board_view">조회수 ${dto.view_count}</div>
					                                <div class="col-6 text-right" id="cmt_sdate">답변 작성 ${dto.sdate}</div>
			                                	</c:otherwise>
			                                </c:choose>
			                            </div>
		                        	</c:forEach>
	                    ...
	                    <div class="col-12 col-md-4 order-1 right">
	                        <div class="top-btn">
	                            <div class="row">
	                                <div class="col-12">
	                                	<i class="fas fa-pen-nib fa-2x"></i><br>
	                                	상담글 작성하고<br>
	                                	무료로 변호사의 답변 받는<br>
	                                    <button type="button" class="toWrite text-right" id="toWrite"><h4>상담글 작성하기 <i class='fas fa-chevron-right'></i> </h4></button>
	                                </div>
	                            </div>
	                        </div>
	                    </div>
