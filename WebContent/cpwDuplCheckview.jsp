...
<body>
	<form>
		<div class="form-group">
			<label for="pwDuplcheckInput">비밀번호를 다시 입력해주세요.</label> <input
				type="password" class="form-control" name="pwDuplcheck"
				id="pwDuplcheckInput">
		</div>
		<input type='button' id='pwduplcheckB' class='btn btn-primary'
			value='확인'>
	</form>
	<script>
		$("#pwduplcheckB").on("click", function() {
			$.ajax({
				url : 'cpwduplcheck.dupl',
				type : 'post',
				data : {
					pw : $("#pwDuplcheckInput").val()
				}
			}).done(function(resp) {
				resp = JSON.parse(resp);
				if (resp[0].dupl == 'true') {
					alert("비밀번호가 맞습니다.");
					$("#clientModify", opener.document).css("display", 'none');
