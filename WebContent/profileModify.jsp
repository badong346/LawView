...
<body>
	...
	<div class=container>
		...
		<div class=contents>
			...
			<form action=modify.profile?seq=${pdto.seq} method="post"
...
<h3>프로필의 제목을 입력하세요</h3><br>
					<div class="form-group row">
						<div class="col-sm">
							<input id="title" name="title" class="form-control-plaintext"
								type="text" placeholder="제목을 입력하세요." value="${pdto.title}">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<c:forEach var="i" begin="0" step="1" end="14">
							<c:choose>
								...
								<c:when test="${pdto.specialty[i]=='이혼'}">
									<script>
				$(function(){
				$("#customControlValidation1").prop("checked",true);
								...
								<c:when test="${pdto.specialty[i]=='손해배상'}">
									<script>
				$(function(){
				$("#customControlValidation2").prop("checked",true);
								...
								<c:when test="${pdto.specialty[i]=='폭행/협박'}">
									<script>
				$(function(){
				$("#customControlValidation3").prop("checked",true);
								...
								<c:when test="${pdto.specialty[i]=='가사기타'}">
									<script>
				$(function(){
				$("#customControlValidation4").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='성범죄'}">
									<script>
				$(function(){
				$("#customControlValidation5").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='재산범죄'}">
									<script>
				$(function(){
				$("#customControlValidation6").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='수사'}">
									<script>
				$(function(){
				$("#customControlValidation7").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='민사기타'}">
									<script>
				$(function(){
				$("#customControlValidation8").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='친족'}">
									<script>
				$(function(){
				$("#customControlValidation9").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='채권추심'}">
									<script>
				$(function(){
				$("#customControlValidation10").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='민사소송절차'}">
									<script>
				$(function(){
				$("#customControlValidation11").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='상속'}">
									<script>
				$(function(){
				$("#customControlValidation12").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='형사기타'}">
									<script>
				$(function(){
				$("#customControlValidation13").prop("checked",true);
				})
				</script>
								</c:when>
								<c:when test="${pdto.specialty[i]=='기타법률서비스'}">
									<script>
				$(function(){
				$("#customControlValidation14").prop("checked",true);
				})
				</script>
								</c:when>
							</c:choose>
					...
					<div class="form-group">
						<textarea class="form-control" name="education"
							...
							placeholder="education 수정">${pdto.education}</textarea>
					</div>
					...
					<div class="form-group">
						...
						<br>
						<div class=thumbnail>
							<img src="${pageContext.request.contextPath}/upload/${pdto.img}"
					...
					<div id="modifyBtn">
						<input type="submit" id="write" value="수정 완료"> <input
