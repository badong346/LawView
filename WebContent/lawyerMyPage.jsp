...
<body>
	...
	<div class=contents>
		<div id="signUp">
			<h4>
				회원정보<br>
				<form action="lawyerModidel.mypage" method="post" id="frm">
					<div class="form-group pwform" >
						<b class="necessary">*</b><label for="pw">비밀번호</label> <input
							type="password" class="form-control" id="pw" name="pw"
							aria-describedby="pwHelp" placeholder="8~16자 단어(영대문자 숫자 언더바 포함)"
							readonly="true" value=""> <small id="pwHelp"
							class="form-text text-muted regex"></small>
					</div>
					<div class="form-group pwform">
						<b class="necessary">*</b><label for="pwConfirm">비밀번호 확인</label> <input
							type="password" class="form-control" id="pwConfirm"
							aria-describedby="pwConfirmHelp" placeholder="비밀번호를 한번 더 입력해주세요"
							readonly="true" value=""> <small id="pwConfirmHelp"
							class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="name">이름</label> <input
							type="text" class="form-control" id="name" name="name"
							aria-describedby="nameHelp" placeholder="한글" readonly="true"
							value="${ldto.name}"> <small id="nameHelp"
							class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="email">이메일</label> <input
							type="email" class="form-control" id="email" name="email"
							aria-describedby="emailHelp" placeholder="name@example.com"
							readonly="true" value="${ldto.email}"> <small
							id="emailHelp" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="phone">전화번호</label> <input
							type="text" class="form-control" id="phone" name="phone"
							aria-describedby="phoneHelp" placeholder="-제외한 숫자 9~11 자리수"
							readonly="true" value="${ldto.phone}"> <small
							id="phoneHelp" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="office_name">사무실 이름</label>
						<input type="text" class="form-control" id="office_name"
							name="office_name" aria-describedby="office_nameHelp"
							placeholder="한글,영어,숫자 1자 이상" readonly="true"
							value="${ldto.office_name}"> <small id="office_nameHelp"
							class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="office_phone">사무실
							전화번호</label> <input type="text" class="form-control" id="office_phone"
							name="office_phone" aria-describedby="office_phoneHelp"
							placeholder="-제외한 숫자 9~11 자리수" readonly="true"
							value="${ldto.office_phone}"> <small
							id="office_phoneHelp" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="office_zipcode">사무실
							우편번호</label>
						<button type="button" id="zipcodeB"
							class="btn btn-outline-primary addr"
							onclick="sample4_execDaumPostcode()" disabled>주소찾기</button>
						<input type="text" class="form-control" id="office_zipcode"
							name="office_zipcode" aria-describedby="office_zipcodeHelp"
							placeholder="" readonly value="${ldto.office_zipcode}"> <small
							id="office_zipcodeHelp" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="office_addr1">사무실 주소</label>
						<input type="text" class="form-control" id="office_addr1"
							name="office_addr1" aria-describedby="office_addr1Help"
							placeholder="" readonly value="${ldto.office_addr1}"> <small
							id="office_addr1Help" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="office_addr2">사무실
							상세주소</label> <input type="text" class="form-control" id="office_addr2"
							name="office_addr2" aria-describedby="office_addr2Help"
							placeholder="" readonly value="${ldto.office_addr2}"> <small
							id="office_addr2Help" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="test">출신 시험</label> <select
							id="test" name="test" class="form-control"
							aria-describedby="testHelp" disabled>
							<option value="" hidden>시험 선택</option>
							<option value="test1"
								<c:if test="${ldto.test eq 'test1'}">selected</c:if>>사법시험</option>
							<option value="test2"
								<c:if test="${ldto.test eq 'test2'}">selected</c:if>>변호사시험</option>
							<option value="test3"
								<c:if test="${ldto.test eq 'test3'}">selected</c:if>>군법무관
								임용시험</option>
							<option value="test4"
								<c:if test="${ldto.test eq 'test4'}">selected</c:if>>고등고시</option>
						</select> <small id="testHelp" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						<b class="necessary">*</b><label for="test_num">시험 횟수</label> <select
							id="test_num" name="test_num" class="form-control"
							aria-describedby="test_numHelp" disabled>
							<option value="" hidden selected>몇 회</option>
							<option value="1"
								<c:if test="${ldto.test_num eq 1}">selected</c:if>>1회</option>
							<option value="2"
								<c:if test="${ldto.test_num eq 2}">selected</c:if>>2회</option>
							<option value="3"
								<c:if test="${ldto.test_num eq 3}">selected</c:if>>3회</option>
							<option value="4"
								<c:if test="${ldto.test_num eq 4}">selected</c:if>>4회</option>
							<option value="5"
								<c:if test="${ldto.test_num eq 5}">selected</c:if>>5회</option>
							<option value="6"
								<c:if test="${ldto.test_num eq 6}">selected</c:if>>6회</option>
							<option value="7"
								<c:if test="${ldto.test_num eq 7}">selected</c:if>>7회</option>
							<option value="8"
								<c:if test="${ldto.test_num eq 8}">selected</c:if>>8회</option>
						</select> <small id="test_numHelp" class="form-text text-muted regex"></small>
					</div>
					<div class="form-group">
						성별<br>
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" name="gender"
								id="male" value="male" disabled
								<c:if test="${ldto.gender eq 'male'}">checked</c:if>> <label
								class="form-check-label" for="male">남성</label>
						</div>
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" name="gender"
								id="female" value="female" disabled
								<c:if test="${ldto.gender eq 'female'}">checked</c:if>>
					...
					<div class="form-group">
						<label for="birth">생년월일</label> <input type="text"
							...
							aria-describedby="birthHelp" placeholder="8 자리수 ex)20200514"
							readonly value="${ldto.birth}"> <small id="birthHelp"
							class="form-text text-muted regex"></small>
					</div>
					<input type="button" id="lawModify" class="btn btn-primary"
						value="정보수정"> <input type="submit" id="modi_del" name="modi_del"
						class="btn btn-primary" value="정보수정 완료"> <input
						type="button" id="delete" name="modi_del" class="btn btn-primary"
						value="회원탈퇴">
				</form>
				<div class=col-12>
					<br>
		...
		<div class="container2">
			<div class="row">
				<div class="col-12" align=center><b class=p-10>내가 쓴 댓글</b></div>
			</div>
			<div class="row">
				...
				<c:otherwise>
						<c:forEach var="i" items="${lList}">
								<div class="row cboard">
							        ...
							        <div class="col-8" align=center><a href="contents.req?seq=" + ${i.seq}>${i.title}</a></div>
	...
	<script>
		$(function() {
			$("#delete").on("click", function() {
				location.href = "lawyerDelete.mypage";
			})
		})
		$(function(){
			$("#lawModify").on("click",function(){
				window.open("lpwDuplCheckview.jsp","","width=500, height=500");
			})
		})
		/*
		$(function(){
        		$("#lawModify").on("click",function(){
        			$("#lawModify").css("display", 'none');
    				...
    				$("#office_name").attr("readonly", false);
    				$("#office_phone").attr("readonly", false);
    				$("#zipcodeB").attr("disabled", false);
    				$("#test").attr("disabled", false);
    				$("#test_num").attr("disabled", false);
