...
<script>
	$(function() {
		...
		$("#reservDate").on("click", function() {
			location.href = "toReservDate.profile?seq=" + ${pdto.seq};
		})
		...
		<div class="row">
			<div class="col-12" align=left>
				<b> ${pdto.name}변호사의 전문 분야   </b><br>
			  	 <c:forEach var="i" begin="0" step="1" end="${fn:length(pdto.specialty)}">
				 ${pdto.specialty[i]}
				 </c:forEach>
			</div>
		</div>
				...
				<c:when test="${pdto.id == sessionScope.lawyerInfo}">
					<div class="col-12" align=right>
						<input type=button id=back value="프로필 목록으로"> <input
							...
							id=modify value="프로필 수정"> <input type=button
							id=reservDate value="상담시간 설정">
					</div>
				</c:when>
